...
public class TwofishManaged : Twofish
{
	...
	internal TwofishManaged(bool skipFipsCheck)
	{
	...
	public override ICryptoTransform CreateEncryptor(byte[] rgbKey, byte[] rgbIV)
	{
		...
		return new gpvmk(new shxuy(rgbKey), rgbIV, ModeValue, PaddingValue, encrypt: true);
	}

	public override ICryptoTransform CreateDecryptor(byte[] rgbKey, byte[] rgbIV)
	{
		CryptoHelper.hzwkb(ref rgbKey, ref rgbIV, KeySizeValue, BlockSizeValue, ModeValue, LegalKeySizesValue);
		return new gpvmk(new shxuy(rgbKey), rgbIV, ModeValue, PaddingValue, encrypt: false);
	}
}
